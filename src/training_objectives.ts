...
export const TRAINING_OBJECTIVES: TrainingObjective[] = [
  {
    ...
    warmupGoal: "{{Z2_MIN}}",
    mainGoal: "{{Z2_MIN}}-{{Z2_MAX}}",
    recoveryGoal: "{{Z1_MIN}}-{{Z1_MAX}}"
  },
  {
    ...
    warmupGoal: "{{Z2_MIN}}",
    mainGoal: "{{Z1_MIN}}-{{Z3_MAX}}",
    recoveryGoal: "{{Z1_MIN}}-{{Z1_MAX}}"
  },
  {
    title: "Loose Anaerobic Interval",
    targetZones: [4, 5],
    transitionStrategy: "interval state",
    mission: "ANAEROBIC (Zone 4/5 Push)\nBIOMETRIC MAP\nMHR: {{MHR}} BPM\nZ1: {{Z1_MIN}}-{{Z1_MAX}} BPM\nZ2: {{Z2_MIN}}-{{Z2_MAX}} BPM\nZ3: {{Z3_MIN}}-{{Z3_MAX}} BPM\nZ4: {{Z4_MIN}}-{{Z4_MAX}} BPM\nZ5: {{Z5_MIN}}-{{Z5_MAX}} BPM\nPRIMARY DIRECTIVE\nTarget: High-Intensity Interval Peaks ({{Z4_MIN}}-{{Z5_MAX}} BPM)\nInterval Floor: >{{Z4_MIN}} BPM\nSuccess: Completion of 3 targeted spikes with full recovery.\nPHASE PROTOCOLS (Iterative Loop)\nWARMUP: Transition to {{Z2_MIN}} BPM. Goal: Reach target within 5:00.\nMAIN_ACTIVE (Spike): Push HR > {{Z4_MIN}} BPM.\nRECOVERY (Valley): Drop HR < {{Z3_MAX}} BPM.\nADHERENCE LOGIC\nRating: 'Good' if user breaches the {{Z4_MIN}} floor during the Spike phases and successfully drops below the {{Z3_MAX}} ceiling during the Valley phases.\nSAFETY & RECOVERY\nFinal Recovery Ceiling: {{Z3_MAX}} + {{BUFF_WIDTH}} BPM (Ensure aerobic baseline is restored before next interval).\nPeak HR: {{MHR}} BPM.\nAlert: Warning - Absolute maximum heart rate reached. Abort interval and immediately drop to Zone 1.",
    warmupGoal: "{{Z4_MIN}}",
    mainGoal: "{{Z4_MIN}}-{{Z5_MAX}}",
    recoveryGoal: "{{Z3_MIN}}-{{Z3_MAX}}"
  },
  {
    title: "Fixed Anaerobic Interval",
    targetZones: [4, 5],
    transitionStrategy: "fixed interval state",
    mission: "FIXED ANAEROBIC (Zone 4/5 Push)\nBIOMETRIC MAP\nMHR: {{MHR}} BPM\nZ1: {{Z1_MIN}}-{{Z1_MAX}} BPM\nZ2: {{Z2_MIN}}-{{Z2_MAX}} BPM\nZ3: {{Z3_MIN}}-{{Z3_MAX}} BPM\nZ4: {{Z4_MIN}}-{{Z4_MAX}} BPM\nZ5: {{Z5_MIN}}-{{Z5_MAX}} BPM\nPRIMARY DIRECTIVE\nTarget: High-Intensity Interval Peaks ({{Z4_MIN}}-{{Z5_MAX}} BPM)\nInterval Floor: >{{Z4_MIN}} BPM\nSuccess: Completion of {{COUNT}} targeted spikes of {{TIME}} minutes each.\nPHASE PROTOCOLS (Iterative Loop)\nWARMUP: Transition to {{Z2_MIN}} BPM. Goal: Reach target within 5:00.\nMAIN_ACTIVE (Spike): Push HR > {{Z4_MIN}} BPM for {{TIME}} minutes.\nRECOVERY (Valley): Drop HR < {{Z3_MAX}} BPM for {{TIME}} minutes.\nADHERENCE LOGIC\nRating: 'Good' if user breaches the {{Z4_MIN}} floor during the Spike phases and successfully drops below the {{Z3_MAX}} ceiling during the Valley phases.\nSAFETY & RECOVERY\nFinal Recovery Ceiling: {{Z2_MAX}} + {{BUFF_WIDTH}} BPM (Ensure aerobic baseline is restored before next interval).\nPeak HR: {{MHR}} BPM.\nAlert: Warning - Absolute maximum heart rate reached. Abort interval and immediately drop to Zone 1.",
    warmupGoal: "{{Z4_MIN}}",
    mainGoal: "{{Z4_MIN}}-{{Z5_MAX}}",
    recoveryGoal: "{{Z3_MIN}}-{{Z3_MAX}}"
  }
