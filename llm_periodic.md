...
Provides the main high-level narrative context extracted from the step-1 generation.

- **[THEME]**: The active thematic setting.
- **[MAGUFFIN]**: The specific pursued/thematic object.
- **[ANTAGONIST]**: The main opponent's name/title.
- **[PROTAGONIST]**: The user's thematic role.

## 4. Milestone Section (`<milestone>`)
Conditional block populated only when a narrative event is active for the current packet. Placed directly after the `<narrative_mission_plan>` section. Includes the event title and narrative context (excludes the timing/timestamp).
