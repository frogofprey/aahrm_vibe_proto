...
*   **Narrative Mission Plan**: Upon session initialization, the system must generate a "Narrative Mission Plan" using the selected Persona and the **Google Gemini API** (`gemini-3-flash-preview`).
...
*   **AI Analysis**: Send the Minute Packet (plus Short-Term History context) to the **Google Gemini API** (`gemma-4-31b-it`) to generate a concise, goal-oriented coaching insight. The prompt is streamlined to focus on immediate telemetry and narrative immersion:
    ...
    *   **Current Minute Restructuring**: The `<current_minute_packet>` provides current coaching HR (Smoothed BPM), average/max/min metrics, coaching direction, and an importance score (1-10) in a concise slash-separated format.
...
*   **Visual Metaphor**: The dashboard should be high-contrast, dark-mode, and use "Data-Grid" aesthetics.
*   **Animation**: Use `motion` for HR number updates and chart transitions.
*   **Uplink Status**: Explicit indicators for Bluetooth/WS connection status.
*   **System Logs**: A scrollable log of system events (AI requests, Handshakes) for debugging and immersion.
